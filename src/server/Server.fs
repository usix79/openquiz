...
let appRouter env =
    choose
        [ route "/ping" >=> text ("root pong")
          route "/api/login" >=> loginHandler env
          route "/api/ping" >=> text ("api pong")
...
let serilogConfig =
    { SerilogConfig.defaults with
        ...
        IgnoredResponseFields = Ignore.fromResponse |> Field.responseContentType }

let appRouterWithLogging env =
    SerilogAdapter.Enable(appRouter env, serilogConfig)
...
let buildEnvironment envName logger (cfg: IConfiguration) =
    ...
    { new Env.IAppEnv with
        ...
        member _.AnswersAgent cmd = answersAgent.Post cmd }
    ...
    let host =
        WebHost
            ...
            .ConfigureAppConfiguration(fun ctx builder ->
                printfn
                    "Configuring App:%A Env: %A"
                    ctx.HostingEnvironment.ApplicationName
                    ctx.HostingEnvironment.EnvironmentName

                builder.AddSystemsManager("/OpenQuiz/" + ctx.HostingEnvironment.EnvironmentName)
                |> ignore)
            .Configure(fun ctx app ->
                ...
                let env =
                    buildEnvironment ctx.HostingEnvironment.EnvironmentName logger ctx.Configuration

                Data2.Quizzes.subscribe (fun quiz ->
                    let event' = Presenter.quizChangeEvent quiz
                    env.Logger.Information("{@Op} {@Evt}", "Event", event')

                    Aws.publishQuizMessage env quiz.Dsc.QuizId quiz.Dsc.ListenToken quiz.Version event')
