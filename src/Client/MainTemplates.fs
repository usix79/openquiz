...
let termsOfUse dispatch acceptMsg cancelMsg =
    div
        ...
        [ div [ Class "modal-background" ] []
          div
              [ Class "modal-card"; Style [ Height "70%"; Width "80%" ] ]
              [ header
                ...
                footer
                    ...
                    [ button
                          [ Class "button is-black has-text-danger"
                            OnClick(fun _ -> dispatch acceptMsg) ]
                          [ str "Accept" ]
...
let inputModal dispatch title txt changeMsg okMsg cancelMsg =
    div
        ...
        [ div [ Class "modal-background" ] []
          div
              [ Class "modal-card" ]
              [ header [ Class "modal-card-head" ] [ p [ Class "modal-card-title" ] [ str title ] ]
                section
                    [ Class "modal-card-body" ]
                    [ input
                          [ Class "input"
                            AutoFocus true
                            valueOrDefault txt
                            OnChange(fun ev -> changeMsg ev.Value |> dispatch) ] ]
                footer
                    [ Class "modal-card-foot" ]
...
let imgArea' style tag disabled changeMsg clearMsg mediaHost imgKey defaultImg clearText =
    [ if not (System.String.IsNullOrWhiteSpace imgKey) then
          figure
              [ classList [ "image", true; style, true ]; Style [ MaxWidth "320px" ] ]
      ...
      div
          [ Class "file"; Style [ MarginTop "8px" ] ]
          [ label
                [ Class "file-label" ]
                [ input
                      [ Class "file-input"
...
let mediaArea tag disabled urlMsg mediaMsg changeMsg clearMsg mediaHost (media: Shared.MediaDsc option) clearText =
    [ let url =
          media
          |> Option.map (fun m -> Shared.Infra.urlForMedia mediaHost m.Key)
          |> Option.defaultValue ""

      match media with
      | Some media ->
          match media.Type with
          | Shared.Picture -> figure [ Class "image"; Style [ MaxWidth "320px" ] ] [ img [ Src url ] ]
          | Shared.Audio -> ReactPlayer.playerEx false false url
          | Shared.Video -> ReactPlayer.playerEx true false url
      | None -> ()
      ...
      div
          ...
          [ input
                [ Class "input is-link is-small"
                  ...
                  valueOrDefault url
                  OnChange(fun ev -> urlMsg ev.Value) ]
            div
                [ Class "select is-small" ]
                [ select
                      [ OnChange(fun ev -> mediaMsg ev.Value) ]
                      [ option [ Selected media.IsNone ] [ str "..." ]
                        option
                            [ Selected(
                                  match media with
                                  | Some { Type = Shared.Picture } -> true
                                  | _ -> false
                              ) ]
                            [ str "picture" ]
                        option
            ...
            | Shared.Picture ->
                figure
                    [ Class "image"
                      Style [ MaxWidth "320px"; Display DisplayOptions.InlineBlock ] ]
                    [ img [ Src url ] ]
            | Shared.Audio -> ReactPlayer.playerEx false isPlaying url
            | Shared.Video -> ReactPlayer.playerEx true isPlaying url
        | None -> ()
    }
...
let footerHero =
    div [ Class "hero-foot has-background-dark has-text-grey-light" ] [ footer ]

let footerFixed =
    div
        [ Class "has-background-dark has-text-grey-light"
          Style
    ...
    div
        [ Style
              [ Position PositionOptions.Fixed
                ...
                OverflowX OverflowOptions.Hidden ] ]
        [ div
              [ Class "tabs is-white is-large is-toggle is-fullwidth" ]
              [ ul
                    []
                    [ li
                          [ classList [ "has-text-weight-bold", current = history ] ]
                          [ a [ OnClick(fun _ -> dispatch history) ] [ str l10n.MenuHistory ] ]
                      li
                          [ classList [ "has-text-weight-bold", current = questions ]
                            Style [ Height "100%"; Width "33%" ] ]
                          [ a
                                [ Style
                                      [ Height "100%"
                                        JustifyContent(if isLastCall then "left" else "center")
                                        Padding "0" ]
                                  OnClick(fun _ -> dispatch questions) ]
                                [ if isCountdownActive then
                                      if secondsLeft > 10 then
                                          str <| (secondsLeft - 10).ToString()
                                      else
                                          div
                                              [ Class "has-background-danger"
                                                Style
                                                    [ Height "100%"
                                                      Width(sprintf "%i%%" ((10 - secondsLeft + 1) * 10)) ] ]
                                              [ span
                                                    [ Class "is-overlay has-text-weight-bold"
                                                      Style [ Padding "12px" ] ]
                                                    [ str l10n.MenuLastCall ] ]
                                  else
                                      str l10n.MenuQuestion ] ]
                      li
                          [ classList [ "has-text-weight-bold", current = results ] ]
                          [ a [ OnClick(fun _ -> dispatch results) ] [ str l10n.MenuResults ] ] ] ] ]

let playSounds mediaHost secondsLeft =
    if secondsLeft = 11 then
        Infra.play "/chgk2-sig2.mp3"

    if secondsLeft = 1 then
        Infra.play "/chgk2-sig3.mp3"
          ...
          | None ->
              ...
              | None ->
                  ...
                  figure
                      ...
                      [ img [ Src <| Shared.Infra.urlForMediaImgSafe mediaHost quizImg ] ]
...
let resultsViewEmb' full quizId resultsToken teamId mediaHost =
    div
        [ Class "content"; Style [] ]
        [ let height = if full then "80vh" else "60vh"

          iframe
              [ Src(Common.urlForResultsIframe quizId resultsToken teamId mediaHost)
                Style
                    [ Width "100%"
                      Height height
                ...
                p
                    ...
                    [ button
                          [ classList
                            ...
                            OnClick(fun _ -> dispatch <| deleteMsg) ]
                          [ str "Delete" ] ]
                p
                    [ Class "control" ]
