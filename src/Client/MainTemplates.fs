...
let mediaArea tag disabled urlMsg mediaMsg changeMsg clearMsg mediaHost (media:Shared.MediaDsc option) clearText =
    [
        ...
        div [Class "field has-addons"] [
            input [Class "input is-link is-small"; Type "text"; Placeholder "Media Url"; valueOrDefault  url; OnChange (fun ev -> urlMsg ev.Value) ]
            div [Class "select is-small"] [
                select [OnChange (fun ev -> mediaMsg ev.Value)] [
                    option [Selected media.IsNone] [ str "..." ]
                    option [Selected (match media with Some {Type = Shared.Picture} -> true | _ -> false)] [ str "picture" ]
                    option [Selected (match media with Some {Type = Shared.Audio} -> true | _ -> false)] [ str "audio" ]
                    option [Selected (match media with Some {Type = Shared.Video} -> true | _ -> false)] [ str "video" ]
                ]
            ]
        ]
...
let resultsViewEmb' full quizId resultsToken teamId mediaHost =
    div [Class "content"; Style []] [
        ...
        iframe [Src (Common.urlForResultsIframe quizId resultsToken teamId mediaHost); Style [Width "100%"; Height height; OverflowX OverflowOptions.Visible; OverflowY OverflowOptions.Visible]] []
    ]
