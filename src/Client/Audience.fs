...
open OpenQuiz.Shared.AudModels
open Common
...
let update (api: IAudApi) (user: AudUser) (msg: Msg) (cm: Model) : Model * Cmd<Msg> =
    ...
    | ChangeTab History ->
        { cm with ActiveTab = History }
        |> ok
        |> apiCmd api.getHistory () GetHistoryResp Exn
          ...
          tbody
              ...
              [ for aw in model.History |> List.rev do
                    ...
                    tr
                        []
                        [ td [] [ p [] [ str aw.QwName ] ]
                          td [] [ p [ Class "is-italic" ] [ str (aw.QwAw.Split('\n').[0]) ] ]
                          td [] [] ] ] ]