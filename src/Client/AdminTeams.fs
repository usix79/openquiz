...
let createCmd (api: IAdminApi) (model: Model) =
    ...
    | Some txt ->
        { model with CreateDlg = None }
        |> apiCmd api.createTeam {| TeamName = txt |} CreateTeamResp Exn
    | None -> model |> noCmd
...
let toggleCard (api: IAdminApi) teamId model =
    ...
    | None ->
        ...
        | _ ->
            { model with Card = None }
            |> loading teamId
            |> apiCmd api.getTeamCard {| TeamId = teamId |} GetTeamResp Exn
...
let validate (card: TeamCard) =
    [ validateName card.TeamName ] |> List.filter (fun s -> s <> "")

let submitCard api model =
    match model.Card with
    | Some card -> model |> loading card.TeamId |> apiCmd api.updateTeamCard card SubmitResp Exn
    | None -> model |> noCmd

let replaceRecord record model : Model =
    { model with
        Teams = record :: (model.Teams |> List.filter (fun q -> q.TeamId <> record.TeamId)) }
...
let init (api: IAdminApi) user : Model * Cmd<Msg> =
    { Errors = Map.empty
      ...
      Card = None
      CreateDlg = None }
    |> apiCmd api.getTeams () GetTeamsResp Exn
...
let update (api: IAdminApi) user (msg: Msg) (cm: Model) : Model * Cmd<Msg> =
    ...
    | CreateTeamResp { Value = Ok res } ->
        { cm with
            Teams = res.Record :: cm.Teams }
        |> noCmd
    ...
    | UpdateStatus txt ->
        ...
        |> updateCard (fun c ->
            { c with
                TeamStatus = defaultArg (fromString txt) New })
        |> noCmd
              ...
              [ thead
                ...
                tbody
                    ...
                    [ for team in model.Teams |> List.sortBy (fun q -> q.TeamId) |> List.rev do
                          let hasCard =
                              match model.Card with
                              | Some card when card.TeamId = team.TeamId -> true
                              | _ -> false

                          let isLoading =
                              match model.CardIsLoading with
                              | Some id when id = team.TeamId -> true
                              | _ -> false

                          tr
                              [ classList
                                    [ "has-text-weight-bold", team.TeamStatus = New
                                      "has-text-danger", team.TeamStatus = Rejected ] ]
                              [ th
                                    []
                                    [ button
                                          [ classList [ "button", true; "is-small", true; "is-loading", isLoading ]
                                            OnClick(fun _ -> dispatch <| ToggleCard team.TeamId) ]
                                            ...
                                            if (team.TeamStatus = New) then
                                                ...
                                                button
                                                    [ Class "button is-small is-white"
                                                      Title "Admit"
                                                      OnClick(fun _ -> ChangeStatus(team.TeamId, Admitted) |> dispatch) ]
                                                    [ span
                                                ...
                                                button
                                                    [ Class "button is-small is-white"
                                                      Title "Reject"
                                                      OnClick(fun _ -> ChangeStatus(team.TeamId, Rejected) |> dispatch) ]
                                                    [ span
                                                          [ Class "icon has-text-danger" ]
                                                          [ Fa.i [ Fa.Regular.ThumbsDown ] [] ] ] ] ] ]

                          if hasCard then
                              tr [] [ th [] []; td [ ColSpan 4 ] [ card dispatch model.Card.Value isLoading ] ] ] ]
                          ...
                          [ label [ Class "label" ] [ str "Entry token" ]
                            div
                                ...
                                [ input
                                      [ Class "input"
                                        Type "text"
                                        valueOrDefault card.EntryToken
                                        OnChange(fun ev -> dispatch <| UpdateToken ev.Value) ] ] ] ]
                div
                    [ Class "column" ]
                    [ div
                          [ Class "control" ]
                          [ label [ Class "label" ] [ str "Status" ]
                            div
                                [ Class "select" ]
                                [ select
                                      [ valueOrDefault (card.TeamStatus.ToString())
                                        OnChange(fun ev -> dispatch <| UpdateStatus ev.Value) ]
