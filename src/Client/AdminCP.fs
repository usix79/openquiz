...
open OpenQuiz.Shared.AdminModels
open Common
...
let setPackage api (txt: string) model =
    match Int32.TryParse txt with
    | true, id ->
        model
        |> loading
        |> apiCmd api.setPackage {| PackageId = if id <> -1 then Some id else None |} QuizCardResp Exn
    | _ -> model |> noCmd

let setSlipIdx (txt: string) model =
    match Int32.TryParse txt with
    | true, id ->
        ...
        | Some pkg ->
            ...
            |> updateCard (fun q ->
                { q with
                    PackageSlipIdx = if id <> -1 then Some id else None })
            |> (fun model ->
                ...
                | Some slip ->
                    ...
                    |> updateTour (fun q ->
                        { q with
                            ...
                            Name = if slip.Caption <> "" then slip.Caption else q.Name })
                | None -> model)
        | None -> model
    | _ -> model
...
let setCard api card st model =
    let model =
        { model with
            Quiz = Some card
            TimeDiff = timeDiff st }

    match model.Package, card.PackageId with
    | None, Some id ->
        model
        |> loading
        |> apiCmd api.getPackageCard {| PackageId = id |} PackagesCardResp Exn
    | Some pkg, Some id when pkg.PackageId = id -> model |> editing |> noCmd
    | Some _, Some id ->
        { model with Package = None }
        |> loading
        |> apiCmd api.getPackageCard {| PackageId = id |} PackagesCardResp Exn
...
let updateTour (f: TourControlCard -> TourControlCard) model =
    ...
    | Some quiz ->
        ...
        | Some qw ->
            { model with
                Quiz = Some { quiz with CurrentTour = Some <| f qw } }
        | None -> model
    | _ -> model
...
let scheduleTick (model: Model, cmd: Cmd<Msg>) =
    ...
    | Some quiz when quiz.QuizStatus = Live ->
        match quiz.CurrentTour with
        | Some tour when tour.IsCoundownActive(serverTime model.TimeDiff) ->
            model, Cmd.batch [ cmd; timeoutCmd Tick 1000 ]
        | _ -> model, cmd
    | _ -> model, cmd
...
let init (api: IAdminApi) user : Model * Cmd<Msg> =
    ...
    |> apiCmd api.getQuizCard () QuizCardResp Exn
...
let update (api: IAdminApi) user (msg: Msg) (cm: Model) : Model * Cmd<Msg> =
    ...
    | ChangeStatus txt ->
        cm
        |> loading
        |> apiCmd api.changeQuizStatus {| QuizStatus = (defaultArg (fromString txt) Setup) |} QuizCardResp Exn
    | ChangeStreamUrl txt -> cm |> loading |> apiCmd api.changeStreamUrl txt QuizCardResp Exn
    | PackagesClick -> cm |> uploadPackages api
    | PackagesResp { Value = Ok pkgs } ->
        { cm with
            AvailablePackages = Some pkgs }
        |> editing
        |> noCmd
        ...
        |> updateTour (fun qw ->
            { qw with
                Slip =
                    match qw.Slip with
                    | Single _ -> qw.Slip
                    | Multiple(name, slips) -> (txt, slips) |> Multiple })
        |> noCmd
    ...
    | DisplayMedia -> cm |> loading |> apiCmd api.showMedia cm.Quiz.Value QuizCardResp Exn
    ...
    div
        ...
        [ div
              ...
              [ div
                    ...
                    [ label [ Class "label" ] [ str "Status" ]
                      div
                          [ Class "select" ]
                          [ select
                                [ Disabled(model.IsLoading || isCountdown)
                                  valueOrDefault (quiz.QuizStatus.ToString())
                                  OnChange(fun ev -> dispatch <| ChangeStatus ev.Value) ]
          ...
          div
              ...
              [ div
                    [ Class "control" ]
                    [ let isControlReadOnly =
                          changesNotAllowed && (not model.IsLastSlip) || model.IsLoading
                      ...
                      | Some pkgs ->
                          div
                              ...
                              [ let value =
                                    match model.Package with
                                    | Some pkg -> pkg.PackageId
                                    | None -> -1

                                select
                                    [ Disabled isControlReadOnly
                                      Value value
                                      OnChange(fun ev -> SelectPackage ev.Value |> dispatch) ]
                      ...
                      | None ->
                          div
                              ...
                              [ div
                                    ...
                                    [ let txt =
                                          match model.Package with
                                          | Some pkg -> str pkg.Name
                                          | None -> str "Not Selected"
                                ...
                                div
                                    [ Class "control" ]
                                    [ button
                                          [ Class "button"; OnClick(fun _ -> dispatch PackagesClick) ]
                                          [ Fa.i [ Fa.Solid.EllipsisH ] [] ] ] ] ]
                    ...
                    div
                        ...
                        [ label [ Class "label" ] [ str "Question" ]
                          div
                              [ Class "select" ]
                              [ let value =
                                    match quiz.PackageSlipIdx with
                                    | Some idx -> idx
                                    | None -> -1
                                ...
                                select
                                    [ Disabled isReadOnly
                                      Value value
                                      OnChange(fun ev -> SelectSlipIdx ev.Value |> dispatch) ]
                                    [ option [ Value -1 ] [ str "Not Selected" ]
                                      match model.Package with
                                      | Some pkg ->
                                          for (idx, slip) in pkg.Slips |> List.mapi (fun idx slip -> idx, slip) do
                                              option
                                                  [ Value idx ]
                                                  [ str <| sprintf "%i %s" (idx + 1) (trimEnd 32 "..." slip.Annotation) ]
                                      | None -> () ] ] ]
                | None -> () ]
          div
              []
          ...
          div
              ...
              [ textarea
                    [ Class "textarea"
                      ...
                      MaxLength 512.0
                      valueOrDefault txt
                      OnChange(fun ev -> dispatch <| UpdateQwText(key, ev.Value)) ]
                    [] ] ]

let qwInput dispatch placeholder txt key partIdx anounced =
    div
        [ Class "control has-icons-left has-icons-right"; Style [ MarginBottom "3px" ] ]
        [ input
              [ Class "input"
                Type "text"
                ...
                valueOrDefault txt
                MaxLength 256.0
                OnChange(fun ev -> UpdateQwTextPart(key, partIdx, ev.Value) |> dispatch) ]
          ...
          div
              ...
              [ textarea
                    [ Class "textarea"
                      ...
                      MaxLength 512.0
                      valueOrDefault txt
                      OnChange(fun ev -> dispatch <| UpdateQwAnswer(key, ev.Value)) ]
                    [] ] ]
          ...
          div
              ...
              [ textarea
                    [ Class "textarea"
                      ...
                      MaxLength 1024.0
                      valueOrDefault txt
                      OnChange(fun ev -> dispatch <| UpdateQwComment(key, ev.Value)) ]
                    [] ] ]
                    ...
                    [ label [ Class "label" ] [ str "Points" ]
                      input
                          [ Class "input"
                            ...
                            OnChange(fun ev -> dispatch <| UpdateQwPoints(key, ev.Value)) ] ]
                div
                    [ Class "control" ]
                    [ label [ Class "label" ] [ str "Jeopardy" ]
                      input
                          [ Class "input"
                            ...
                            OnChange(fun ev -> dispatch <| UpdateQwJpdPoints(key, ev.Value)) ] ]
                label
                    [ Class "checkbox" ]
                    [ input
                          [ Type "checkbox"
                            ...
                            Disabled isReadOnly
                            OnChange(fun ev -> UpdateQwWithChoice(key, (ev.Checked)) |> dispatch) ]
                      str " with choice" ]
                    ...
                    label
                        ...
                        [ input
                              [ Type "checkbox"
                                ...
                                Disabled isReadOnly
                                OnChange(fun ev -> UpdateQwEOT(key, (ev.Checked)) |> dispatch) ]
                          str " end of tour" ] ]
              ...
              div
                  ...
                  [ label [ Class "label" ] [ str "Questions Text" ]
                    for (idx, qw) in list |> List.indexed do
                        qwInput
                            dispatch
                            (sprintf "Question part %i" (idx + 1))
                            qw
                            key
                            idx
                            (idx < nextQwPartIdx || status <> Announcing) ]

          yield! MainTemplates.mediaEl settings.MediaHost slip.QuestionMedia false
          br []
          match slip.Answer with
          | OpenAnswer txt -> awTextArea dispatch key txt isReadOnly
          | ChoiceAnswer list ->
              div
                  [ Class "content" ]
                  [ strong [] [ str "Answer" ]
              ...
              [ label [ Class "label" ] [ str "Tour Name" ]
                input
                    [ Class "input"
                      ...
                      Disabled isReadOnly
                      valueOrDefault name
                      OnChange(fun ev -> dispatch <| UpdateSlipName ev.Value) ] ]
          br []
          div
              [ Class "table-container"; Style [ Width "800px" ] ]
              [
                      ...
                      tbody
                          ...
                          [ tr
                                ...
                                [ for (idx, slip) in slips |> List.indexed do
                                      td
                                          [ classList [ "has-background-success", idx = nextQwIdx ] ]
                                          [ singleSlipEl
                                                dispatch
                                                settings
                                                status
...
let qwView
    ...
    div
        ...
        [ div
              ...
              [ div
                    ...
                    [ label [ Class "label is-large" ] [ str "#" ]
                      input
                          [ Class "input is-large"
                            Type "text"
                            Disabled isReadOnly
                            valueOrDefault tour.Name
                            OnChange(fun ev -> dispatch <| UpdateTourName ev.Value) ] ]
                ...
                | Countdown, sec when sec > 0 ->
                    div
                        [ Class "control" ]
                        [ label
                              [ classList [ "label", true; "is-large", true; "has-text-danger", sec <= 10 ] ]
                              [ str "Seconds Left" ]
                          input
                              [ classList [ "input", true; "is-large", true; "has-text-danger", sec <= 10 ]
                                Type "text"
                                Disabled true
                                Value(secondsLeftToString sec) ] ]
                | _ ->
                    div
                        [ Class "control"; Style [ Width "min-content" ] ]
                        [ label [ Class "label is-large" ] [ str "Seconds" ]
                          input
                              [ Class "input is-large"
                                Type "number"
                                Disabled isReadOnly
                                valueOrDefault (tour.Seconds.ToString())
                                OnChange(fun ev -> dispatch <| UpdateTourSeconds ev.Value) ] ] ]
          ...
          | Announcing, _ when tour.IsSingleMultipart ->
              ctrlBtn NextQwPart (sprintf "Show Question Part %i" (tour.QwPartIdx + 1))
          ...
          | Multiple(name, slips) ->
              multipleSlipEl dispatch settings tour.Status name slips tour.QwIdx tour.QwPartIdx isReadOnly ]