...
let toggleCard api quizId model =
    ...
    | None ->
        ...
        | _ ->
            { model with Card = None; Form = None }
            |> loading quizId
            |> apiCmd api.getProdQuizCard {| QuizId = quizId |} GetCardResp Exn
...
let submitCard api model =
    ...
    | Some card ->
        model
        |> loading card.QuizId
        |> apiCmd api.updateProdQuizCard card SubmitCardResp Exn
    | None -> model |> noCmd

let replaceRecord record model =
    { model with
        Quizzes = record :: (model.Quizzes |> List.filter (fun q -> q.QuizId <> record.QuizId)) }
...
let toggleDeleteForm model =
    { model with
        Form = DeleteForm.Toggle model.Form }
...
let afterQuizDeletion (quizId: int) model =
    { model with
        Card = None
        Form = None
        Quizzes = model.Quizzes |> List.filter (fun q -> q.QuizId <> quizId) }
...
let update (api: IMainApi) user (msg: Msg) (cm: Model) : Model * Cmd<Msg> =
    ...
    | DeleteQuiz quizId ->
        ...
        |> apiCmd api.deleteQuiz {| QuizId = quizId |} (taggedMsg DeleteQuizResp quizId) Exn
    | DeleteQuizResp { Tag = quizId; Rsp = { Value = Ok _ } } -> cm |> afterQuizDeletion quizId |> noCmd
    | DeleteQuizResp { Tag = _; Rsp = { Value = Error txt } } ->
        cm
        |> updateDeleteForm (fun form ->
            { form with
                IsSending = false
              ...
              [ div
                ...
                div
                    ...
                    [ p
                          [ Class "level-item" ]
                          [ button
                                [ Class "button is-dark"; OnClick(fun _ -> dispatch CreateQuiz) ]
                                [ str "Create New Quiz" ] ] ] ]
              ...
              [ thead
                ...
                tbody
                    ...
                    [ for quiz in model.Quizzes |> List.sortBy (fun q -> q.QuizId) |> List.rev do
                          let hasCard =
                              match model.Card with
                              | Some card when card.QuizId = quiz.QuizId -> true
                              | _ -> false

                          let isLoading =
                              match model.CardIsLoading with
                              | Some id when id = quiz.QuizId -> true
                              | _ -> false
                              ...
                              [ th
                                    ...
                                    [ button
                                          [ classList [ "button", true; "is-small", true; "is-loading", isLoading ]
                                            OnClick(fun _ -> dispatch <| ToggleCard quiz.QuizId) ]
                                          [ str <| if hasCard then "-" else "+" ] ]
                                th
                                    []
                                    [ str
                                      <| match quiz.StartTime with
                                         | Some st -> st.ToString("dd/MM HH:mm")
                                         | None -> "???" ]
                                td [] [ str quiz.Name ]
                                ...
                                td
                                    []
                                    [ a
                                          [ urlForAdmin quiz.QuizId quiz.AdminToken |> Href; Target "_blank" ]
                      ...
                      div
                          ...
                          [ label [ Class "label" ] [ str "Farewell Message" ]
                            div
                                ...
                                [ textarea
                                      [ Class "textarea"
                                        Placeholder "Say goodbye to your audience"
                                        MaxLength 512.0
                      ...
                      div
                          ...
                          [ label [ Class "label" ] [ str "Registration Message" ]
                            div
                                ...
                                [ textarea
                                      [ Class "textarea"
                                        Placeholder "Instruction for registration"
                                        MaxLength 512.0
                          ...
                          [ label [ Class "label" ] [ str "Event Page" ]
                            div
                                ...
                                [ input
                                      [ Class "input"
                                        Type "text"
                                        Placeholder "Link to facebook event or telegram"
                                        MaxLength 128.0
                      ...
                      div
                          ...
                          [ div
                                ...
                                [ label
                                      ...
                                      [ input
                                            [ Type "checkbox"
                                              Checked card.WithPremoderation
                                              OnChange(fun ev -> dispatch <| UpdateIsPremoderated ev.Checked) ]
                                        str " registration is moderated" ] ] ] ] ]
          match form with
          | Some fm ->
              MainTemplates.deleteForm
                  dispatch
                  "Type name of the Quiz to confirm"
                  card.Name
                  fm.ConfirmText
                  fm.IsSending
                  fm.FormError
