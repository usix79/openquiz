...
type Msg =
    ...
    | QuizImgChanged of {|File:Browser.Types.File; Tag:unit|}
...
let addError txt model = {model with Errors = model.Errors.Add(System.Guid.NewGuid().ToString(),txt)}
let delError id model = {model with Errors = model.Errors.Remove id}
...
let uploadFileS3 (api:IMainApi) (file:Browser.Types.File) model =
    ...
    else model |> loading, uploadFileToS3Cmd api.getUploadUrl QuizImg file QuizImgUploaded Exn
