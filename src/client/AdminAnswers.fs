...
let addError txt model =
    { model with
        Errors = model.Errors.Add(Guid.NewGuid().ToString(), txt) }
...
let setResults (api: IAdminApi) (teamId: int) (key: QwKey) (v: string) (model: Model) =
    ...
    | Some bundle ->
        ...
        | Some aw ->
            let answersToResult =
                if aw.Txt <> "" then
                    bundle.FindAnswers key aw.Txt aw.Jpd
                else
                    [ teamId, aw ]

            let answersToUpdate =
                answersToResult
                |> List.map (fun (teamId, aw) ->
                    teamId,
                    { aw with
                        Res = res
                        IsA = false
                        UT = Some <| serverTime model.TimeDiff })
                |> Map.ofList

            let answersToSend =
                answersToResult
                |> List.map (fun (teamId, _) ->
                    {| QwKey = key
                       Res = res
                       TeamId = teamId |})
...
let rangeFromHashQuery () =
    ...
    if m.Success then
        Some
            { From = Int32.Parse((m.Groups.Item 1).Value)
              To = Int32.Parse((m.Groups.Item 2).Value) }
    else
        None
...
let update (api: IAdminApi) user (msg: Msg) (cm: Model) : Model * Cmd<Msg> =
    match msg with
    | GetAnswersResp { Value = Ok res; ST = st } ->
        { cm with
            Bundle = Some res
    ...
    | ToggleShowAllTeams ->
        { cm with
            ShowAllTeams = not cm.ShowAllTeams }
        |> noCmd
              ...
              [ for qw in bundle.Questions |> List.sortBy (fun q -> q.Key) |> List.rev ->
                    li
                        ...
                        [ let selectedIdx =
                              match model.CurrentQuestion with
                              | Some cqw -> cqw.Key
                              | None -> { TourIdx = -1; QwIdx = -1 }

                          a
                              [ classList [ "light-item", true; "has-background-white", qw.Key = selectedIdx ]
...
let answersTable dispatch (bundle: AnswersBundle) (qw: QuestionRecord) cq showAllTeams =
    [ h5 [ Class "title is-5" ] [ str <| sprintf "Question: %s  \"%s\"" qw.Nm qw.Ann ]
      if qw.Awr <> "" then
          p [ Class "content" ] [ b [] [ str "Answer(s): " ]; p [] (splitByLines qw.Awr) ]
      ...
      label
          ...
          [ input
                [ Type "checkbox"
                  ...
                  OnChange(fun _ -> dispatch ToggleShowAllTeams) ]
            str " show all teams" ]
          ...
          [ thead
            ...
            tbody
                ...
                [ for team, (found, aw) in answers do
                      if found then
                          yield answersRow dispatch team qw aw cq.LastReview
                      else
                          yield noAnswersRow dispatch team qw ] ] ]
...
let answersRow dispatch team (qw: QuestionRecord) (aw: Answer) (lastReview: DateTime) =
    ...
    let modifiers =
        if isNewAnswer aw lastReview then
            ("has-text-weight-bold", true) :: modifiers
        else
            modifiers
        ...
        [ td [] [ span [ classList modifiers ] [ str (team.Id.ToString()) ] ]
          ...
          | Some seconds ->
              td
                  []
                  [ span
                        [ classList [ "has-text-danger", ((int) seconds > (qw.Sec + 20)) ] ]
                        [ str (int(seconds).ToString()) ] ]
          | None -> td [] [] ]
...
let paginator dispatch (bundle: AnswersBundle) (range: Range option) =
    ...
    nav
        [ Class "pagination"; Role "navigation"; AriaLabel "pagination" ]
        [ ul
                ...
                | Some range ->
                    li
                        [ Class "pagination-link"
                          ...
                          OnClick(fun _ -> dispatch <| SelectRange None) ]
                        [ str "All teams" ]
                ...
                | None ->
                    ...
                    if totalIndexes > 0 then
                        ...
                        for idx in 0..totalIndexes do
                            let range =
                                { From = idx * TEAMsOnPAGE + 1
                                  To = (idx + 1) * TEAMsOnPAGE }

                            li
                                [ Class "pagination-link"
                                  Style [ Cursor "pointer" ]
                                  OnClick(fun _ -> Some range |> SelectRange |> dispatch) ]
