...
type Msg =
    ...
    | CreatePackageResp of
        RESP<{| Record: PackageRecord
...
let toggleCard api packageId model =
    ...
    | None ->
        ...
        | Some card when card.PackageId = packageId ->
            { model with
                Card = None
                ShareForm = None }
            |> noCmd
        | _ ->
            { model with
                ...
                ShareForm = None }
            |> loading packageId
            |> apiCmd api.getProdPackageCard {| PackageId = packageId |} GetPackageResp Exn

let editing model = { model with CardIsLoading = None }

let validateName txt =
    if System.String.IsNullOrWhiteSpace(txt) then
        "Name is required"
    else
        ""

let validate (card: MainModels.PackageCard) =
    [ validateName card.Name ] |> List.filter (fun s -> s <> "")
...
let updateSlip (qwKey: QwKey) (f: SingleSlip -> SingleSlip) (model: Model) =
    ...
    |> updateCard (fun card ->
        ...
        | Some(Multiple(name, slips)) ->
            (name, slips |> List.mapi (fun idx slip -> if idx = qwKey.QwIdx then f slip else slip))
            |> Multiple
...
let updateQuestionMedia (qwKey: QwKey) (f: MediaDsc -> MediaDsc) (model: Model) =
    model
    |> updateSlip qwKey (fun slip ->
        { slip with
            QuestionMedia =
                Some
                <| f (slip.QuestionMedia |> Option.defaultWith (fun _ -> { Key = ""; Type = Picture })) })
...
let submitCard api model =
    ...
    | Some card ->
        model
        |> loading card.PackageId
        |> apiCmd api.updateProdPackageCard card SubmitCardResp Exn
    | None -> model |> noCmd
...
let replaceRecord record model =
    { model with
        Packages =
            record
            :: (model.Packages |> List.filter (fun q -> q.PackageId <> record.PackageId)) }

let uploadFile (api: IMainApi) (tag: PkgQwKey) (file: Browser.Types.File) msg model =
    if file.size > (1024 * 1024 * 8) then
        model |> addError "max allowed size is 8Mb" |> noCmd
    else
        model |> loading tag.PackageId, uploadFileToS3Cmd api.getUploadUrl QuestionImg file msg Exn
...
let sendAquiringForm (api: IMainApi) model =
    ...
    | Some form when
        form.PackageId > 0
        && not <| System.String.IsNullOrWhiteSpace(form.TransferToken)
        ->
        model
        ...
        |> apiCmd
            ...
            {| PackageId = form.PackageId
               TransferToken = form.TransferToken.Trim() |}
            AquirePackageResp
            Exn
    | _ -> model |> noCmd
...
let toggleDeleteForm model =
    { model with
        DeleteForm = DeleteForm.Toggle model.DeleteForm }
...
let afterPackageDeletion (packageId: int) model =
    { model with
        ...
        DeleteForm = None
        ShareForm = None
        Packages = model.Packages |> List.filter (fun p -> p.PackageId <> packageId) }
...
let update (api: IMainApi) user (msg: Msg) (cm: Model) : Model * Cmd<Msg> =
    ...
    | CreatePackageResp { Value = Ok res } ->
        { cm with
            Packages = res.Record :: cm.Packages
            Card = Some res.Card }
        |> noCmd
    ...
    | SubmitCardResp { Value = Ok res } ->
        ...
        |> replaceRecord res
        |> noCmd
    | AppendSingleSlip qwCount ->
        cm
        |> updateCard (fun c -> c.AppendSlip(SingleSlip.InitEmpty (c.GetNextSlipCaption()) qwCount |> Single))
        |> noCmd
    ...
    | UpdateTourName(tourIdx, txt) ->
        cm
        |> updateCard (fun c ->
            c.GetSlip tourIdx
            |> Option.bind (fun slip -> slip.SetMultipleName txt |> c.UpdateSlip tourIdx |> Some)
            |> Option.defaultValue c)
        |> noCmd
    | AppendQwToMultiple tourIdx ->
        cm
        |> updateCard (fun c ->
            c.GetSlip tourIdx
            |> Option.bind (fun slip ->
                SingleSlip.InitEmpty (sprintf "%i.%i" (tourIdx + 1) (slip.LastQwIdx + 2)) 1
                |> slip.AppendToMultiple
                |> c.UpdateSlip tourIdx
                |> Some)
            |> Option.defaultValue c)
        |> noCmd
    | DelQwInMultiple key ->
        cm
        |> updateCard (fun c ->
            c.GetSlip key.TourIdx
            |> Option.bind (fun slip -> slip.RemoveFromMultiple key.QwIdx |> c.UpdateSlip key.TourIdx |> Some)
            |> Option.defaultValue c)
        |> noCmd
    ...
    | ToOpenAnswer key ->
        ...
        |> updateSlip key (fun slip ->
            { slip with
                Answer = slip.Answer.ToOpen() })
        |> noCmd
    | ToChoiceAnswer key ->
        cm
        |> updateSlip key (fun slip ->
            { slip with
                Answer = slip.Answer.ToChoice() })
        |> noCmd
    | QwAnswerChanged(key, txt) ->
        cm
        |> updateSlip key (fun slip -> { slip with Answer = OpenAnswer txt })
        |> noCmd
    | AppendChoice key ->
        cm
        |> updateSlip key (fun slip ->
            { slip with
                Answer = slip.Answer.AppendChoice() })
        |> noCmd
    | DeleteChoice(key, idx) ->
        cm
        |> updateSlip key (fun slip ->
            { slip with
                Answer = (slip.Answer.DeleteChoice idx) })
        |> noCmd
    | SetCorrectness(key, idx, isCorrect) ->
        cm
        |> updateSlip key (fun slip ->
            { slip with
                Answer = slip.Answer.SetCorrectness idx isCorrect })
        |> noCmd
    | SetChoiceText(key, idx, txt) ->
        cm
        |> updateSlip key (fun slip ->
            { slip with
                Answer = slip.Answer.SetText idx txt })
        |> noCmd
    | QwCommentChanged(key, txt) -> cm |> updateSlip key (fun slip -> { slip with Comment = txt }) |> noCmd
    | QwPointsChanged(key, txt) ->
        cm
        |> updateSlip key (fun slip ->
            { slip with
                Points = System.Decimal.Parse(txt) })
        |> noCmd
    | QwJpdPointsChanged(key, txt) ->
        cm
        |> updateSlip key (fun slip ->
            { slip with
    ...
    | QwMediaChanged res ->
        cm
        |> uploadFile api res.Tag res.File (fun key -> QwMediaUploaded(res.Tag.Key, key, res.File.``type``))
    | QwMediaClear key -> cm |> updateSlip key (fun slip -> { slip with QuestionMedia = None }) |> noCmd
    | QwMediaUploaded(qwKey, bucketKey, mediaType) ->
        cm
        |> editing
        |> updateSlip qwKey (fun slip ->
            { slip with
                QuestionMedia =
                    Some
                        { Key = bucketKey
                          Type = mediaTypeFromMiME mediaType } })
        |> noCmd
    | QwMediaUrlChanged(key, url) -> cm |> updateQuestionMedia key (fun media -> { media with Key = url }) |> noCmd
    | QwMediaTypeChanged(key, typ) ->
        cm
        |> updateQuestionMedia key (fun media ->
            { media with
                Type = mediaTypeFromMiME typ })
        |> noCmd
    | AnswerMediaChanged res ->
        cm
        |> uploadFile api res.Tag res.File (fun key -> AnswerMediaUploaded(res.Tag.Key, key, res.File.``type``))
    | AnswerMediaClear key -> cm |> updateSlip key (fun slip -> { slip with AnswerMedia = None }) |> noCmd
    | AnswerMediaUploaded(qwKey, bucketKey, mediaType) ->
        cm
        |> editing
        |> updateSlip qwKey (fun slip ->
            { slip with
    ...
    | AnswerMediaTypeChanged(key, typ) ->
        cm
        |> updateAnswerMedia key (fun media ->
            { media with
                Type = mediaTypeFromMiME typ })
        |> noCmd
    | ToggleAquireForm -> cm |> toggleAquiringForm |> noCmd
    | AquireFormUpdatePackageId txt ->
        cm
        |> updateAquiringForm (fun form ->
            { form with
                PackageId = System.Int32.Parse(txt) })
        |> noCmd
    | AquireFormUpdateTransferToken txt ->
        cm
        |> updateAquiringForm (fun form -> { form with TransferToken = txt.Trim() })
        |> noCmd
    | AquireFormCancel -> { cm with AquiringForm = None } |> noCmd
    | AquireFormSend -> cm |> sendAquiringForm api
    | AquirePackageResp { Value = Ok record } ->
        { cm with
            Packages = record :: cm.Packages
            AquiringForm = None }
        |> noCmd
    | AquirePackageResp { Value = Error txt } ->
        cm
        |> addError txt
        |> updateAquiringForm (fun form -> { form with IsSending = false })
        |> noCmd
    | ToggleDeleteForm -> cm |> toggleDeleteForm |> noCmd
    | DeleteFormUpdateText txt ->
        cm
        |> updateDeleteForm (fun form ->
            { form with
                ConfirmText = txt
                FormError = "" })
        |> noCmd
    | DeletePackage packageId ->
        cm
        |> updateDeleteForm (fun form -> { form with IsSending = true })
        |> apiCmd api.deletePackage {| PackageId = packageId |} (taggedMsg DeletePackageResp packageId) Exn
    | DeletePackageResp { Tag = quizId; Rsp = { Value = Ok _ } } -> cm |> afterPackageDeletion quizId |> noCmd
    | DeletePackageResp { Tag = _; Rsp = { Value = Error txt } } ->
        cm
        |> updateDeleteForm (fun form ->
            { form with
                IsSending = false
                ...
                | None ->
                    Some
                        { UserId = ""
                          ...
                          IsSending = false } }
        |> noCmd
    | ShareFormUpdateUserId txt ->
        cm
        |> updateShareForm (fun form -> { form with UserId = txt; Error = "" })
        |> noCmd
    | ShareFormSend pkgId ->
        cm
        |> updateShareForm (fun form -> { form with IsSending = true })
        |> apiCmd
            api.sharePackage
            {| PackageId = pkgId
               UserId = cm.ShareForm.Value.UserId |}
            SharePackageResp
            Exn
    | ShareRemove(pkgId, userId) ->
        cm
        |> apiCmd api.removePackageShare {| PackageId = pkgId; UserId = userId |} (taggedMsg RemoveShareResp userId) Exn
    | SharePackageResp { Value = Error txt } ->
        cm
        |> updateShareForm (fun form ->
            { form with
                IsSending = false
                Error = txt })
        |> noCmd
    | SharePackageResp { Value = Ok res } ->
        { cm with ShareForm = None }
        |> updateCard (fun c ->
            { c with
                SharedWith = res :: c.SharedWith })
        |> noCmd
    | RemoveShareResp { Tag = userId; Rsp = { Value = Ok _ } } ->
        cm
        |> updateCard (fun c ->
            { c with
                SharedWith = c.SharedWith |> List.filter (fun r -> r.Id <> userId) })
        |> noCmd
                ...
                div
                    ...
                    [ if model.AquiringForm.IsNone then
                          p
                              [ Class "level-item" ]
                              [ button
                                    [ Class "button is-dark"; OnClick(fun _ -> dispatch ToggleAquireForm) ]
                                    [ str "Acquire Package" ] ]

                      p
                          [ Class "level-item" ]
                          [ button
                                [ Class "button is-dark"; OnClick(fun _ -> dispatch CreatePackage) ]
                                [ str "Create New Package" ] ] ] ]
              ...
              [ thead
                ...
                tbody
                    ...
                    [ for package in model.Packages |> List.sortBy (fun q -> q.PackageId) |> List.rev do
                          let hasCard =
                              match model.Card with
                              | Some card when card.PackageId = package.PackageId -> true
                              | _ -> false

                          let isLoading =
                              match model.CardIsLoading with
                              | Some id when id = package.PackageId -> true
                              | _ -> false
                          ...
                          tr
                              ...
                              [ th
                                    []
                                    [ button
                                          [ classList [ "button", true; "is-small", true; "is-loading", isLoading ]
                                            OnClick(fun _ -> dispatch <| ToggleCard package.PackageId) ]
                                          [ str <| if hasCard then "-" else "+" ] ]
                                td [] [ str <| package.PackageId.ToString() ]
                                td
                                    []
                                    [ str package.Name
                                      if package.Producer <> user.Sub then
                                          span [ Class "tag is-info" ] [ str "shared with me" ] ] ]
...
let aquiringForm (dispatch: Msg -> unit) (form: AquireForm) =
    div
        [ Class "field has-addons"; Style [ PaddingBottom "5px" ] ]
        [ p
              [ Class "control" ]
              [ input
                    [ Class "input"
                  ...
                  [ div
                        ...
                        [ label [ Class "label" ] [ str "Package Name"; span [ Class "has-text-danger" ] [ str "*" ] ]
                          ...
                          div
                              [ Class "control" ]
                              [ input
                                    [ classList [ "input", true; "is-danger", error <> "" ]
                                      Disabled isLoading
                                      Type "text"
                                      Placeholder "PKG #3"
                                      MaxLength 128.0
                                              ...
                                              [ str record.Name
                                                a
                                                    [ Class "button is-small is-text has-text-danger"
                                                      ...
                                                      OnClick(fun _ ->
                                                          ShareRemove(card.PackageId, record.Id) |> dispatch) ]
                                                    [ Fa.span [ Fa.Size Fa.FaSmall; Fa.Solid.Times ] [] ] ] ] ]

                          match shareForm with
                          | None ->
                              button
                                  [ Class "button is-small"; OnClick(fun _ -> dispatch ToggleShareForm) ]
                                  [ str "Share" ]
                          | Some form ->
                              div
                                  [ Class "field has-addons"; Style [ PaddingBottom "5px" ] ]
                                  [ p
                                    ...
                                    p
                                        ...
                                        [ button
                                              [ classList
                                                    [ "button", true; "is-small", true; "is-loading", form.IsSending ]
                                                Disabled(form.UserId = "")
                                                OnClick(fun _ -> dispatch <| ShareFormSend card.PackageId) ]
                                              [ str "Share" ] ]
                                    p
                                        [ Class "control" ]
                                        [ button
                    ...
                    div
                        ...
                        [ match deleteForm with
                          | Some form ->
                              MainTemplates.deleteForm
                                  dispatch
                                  "Type name of the Package to confirm"
                                  card.Name
                                  form.ConfirmText
                                  form.IsSending
                                  form.FormError
                                  ToggleDeleteForm
                                  DeleteFormUpdateText
                                  (DeletePackage card.PackageId)
                          | None ->
                              div
                                        ...
                                        [ let hasErrors = not (validate card |> List.isEmpty)
                                          ...
                                          button
                                              [ Class "button is-dark "
                                                Disabled hasErrors
                                                OnClick(fun _ -> dispatch SubmitCard) ]
                                              [ str "Submit" ] ]
                                    div
                                        [ Class "control"; Style [ MarginLeft "5px" ] ]
                                        [ button
                                              [ Class "button"; OnClick(fun _ -> dispatch CancelCard) ]
                                              [ str "Cancel" ] ]
                                    button
                                        [ Class "button is-danger"; OnClick(fun _ -> dispatch ToggleDeleteForm) ]
                                        [ str "Delete Package" ] ]
              ...
              div
                  ...
                  [ div
                        ...
                        [ button
                              [ Class "button "
                                Disabled isLoading
                                OnClick(fun _ -> AppendSingleSlip 1 |> dispatch) ]
                              [ str "Append Question" ] ]
                    div
                        [ Class "control" ]
                        [ button
                              [ Class "button "
                                Disabled isLoading
                                OnClick(fun _ -> AppendSingleSlip 2 |> dispatch) ]
                              [ str "Append Doublet" ] ]
                    div
                        [ Class "control" ]
                        [ button
                              [ Class "button "
                                Disabled isLoading
                                OnClick(fun _ -> AppendSingleSlip 3 |> dispatch) ]
                              [ str "Append Blitz" ] ]
                    div
                        [ Class "control" ]
                        [ button
                              [ Class "button "
                                Disabled isLoading
                                OnClick(fun _ -> AppendMultipleSlip |> dispatch) ]
                              [ str "Append Multiple Slip" ] ] ]
              ...
              [ thead
                ...
                tbody
                    ...
                    [ for (slipIdx, slip) in card.Slips |> List.indexed |> List.rev do
                          match slip with
                          | Single s ->
                              yield singleSlipRow dispatch settings isOwned isLoading card.PackageId slipIdx None s
                          | Multiple(name, slips) ->
...
let idxCellInSlip dispatch qwKey slip isLoading =
    td
        ...
        [ div
              ...
              [ input
                    [ Class "input"
                      valueOrDefault slip.Caption
                      MaxLength 4.
                      ReadOnly isLoading
                      OnChange(fun ev -> QwCaptionChanged(qwKey, ev.Value) |> dispatch) ] ]
          label
              [ Class "checkbox"; Title "End Of Tour" ]
              [ input
                    [ Type "checkbox"
                      Checked slip.EndOfTour
                      ReadOnly isLoading
                      OnChange(fun ev -> QwEOTChanged(qwKey, (ev.Checked)) |> dispatch) ]
                str " EOT" ] ]
...
let delTourCell dispatch tourIdx =
    td
        []
        [ button
              [ Class "button is-small"; OnClick(fun _ -> dispatch <| DelSlip(tourIdx)) ]
              [ Fa.i [ Fa.Regular.TrashAlt ] [] ] ]

let delQwCell dispatch qwKey =
    td
        []
        [ button
              [ Class "button is-small"; OnClick(fun _ -> dispatch <| DelQwInMultiple qwKey) ]
              [ Fa.i [ Fa.Regular.TrashAlt ] [] ] ]
...
let qwCell dispatch settings (key: PkgQwKey) txt (media: Shared.MediaDsc option) isLoading =
    td
        ...
        [ textarea
          ...
          br []
          yield!
              MainTemplates.mediaArea
                  key
                  isLoading
                  (fun v -> QwMediaUrlChanged(key.Key, v) |> dispatch)
                  (fun v -> QwMediaTypeChanged(key.Key, v) |> dispatch)
                  (QwMediaChanged >> dispatch)
                  (fun _ -> QwMediaClear key.Key |> dispatch)
                  settings.MediaHost
                  media
                  "Clear" ]
...
let qwInput dispatch placeholder txt key partIdx =
    div
        [ Class "control"; Style [ MarginBottom "3px" ] ]
        [ input
              [ Class "input"
                Type "text"
...
let awCell dispatch (key: PkgQwKey) (answer: SlipAnswer) isLoading =
    td
        ...
        [ div
              ...
              [ button
                    [ classList [ "button", true; "is-small", true; "has-text-weight-bold", (answer.IsOpen()) ]
                      OnClick(fun _ -> dispatch (ToOpenAnswer key.Key)) ]
                    [ str "open answer" ]
                button
                    [ classList
                          [ "button", true
                            "is-small", true
                            "has-text-weight-bold", not (answer.IsOpen()) ]
                      OnClick(fun _ -> dispatch (ToChoiceAnswer key.Key)) ]
                            ...
                            [ div
                                  ...
                                  [ div
                                        ...
                                        [ input
                                              [ Class "input is-small"
                                                Type "text"
                                                valueOrDefault ch.Text
                                                OnChange(fun ev -> SetChoiceText(key.Key, idx, ev.Value) |> dispatch) ] ]
                                    div
                                        [ Class "control" ]
                                        [ a
                                              [ classList
                                                    [ "button", true; "is-small", true; "is-primary", ch.IsCorrect ]
                                                OnClick(fun _ -> SetCorrectness(key.Key, idx, true) |> dispatch) ]
                                              [ str "correct" ] ]
                                    div
                                        [ Class "control" ]
                                        [ a
                                              [ classList
                                                    [ "button", true; "is-small", true; "is-danger", not ch.IsCorrect ]
                                                OnClick(fun _ -> SetCorrectness(key.Key, idx, false) |> dispatch) ]
                                              [ str "wrong" ] ]
                                    div
                                        [ Class "control" ]
                                        [ a
    ...
    td
        ...
        [ textarea
              [ Class "textarea"
                valueOrDefault txt
                MaxLength 1024.0
                OnChange(fun ev -> QwCommentChanged(key.Key, ev.Value) |> dispatch) ]
              []
          br []
          yield!
              MainTemplates.mediaArea
                  key
                  isLoading
                  (fun v -> AnswerMediaUrlChanged(key.Key, v) |> dispatch)
                  (fun v -> AnswerMediaTypeChanged(key.Key, v) |> dispatch)
                  (AnswerMediaChanged >> dispatch)
...
let singleSlipRow dispatch settings isOwned isLoading pkgId tourIdx qwIdx (slip: SingleSlip) =
    ...
    let packageKey =
        { PackageId = pkgId
          TourIdx = tourIdx
          QwIdx = qwIdx |> Option.defaultValue 0 }
                  ...
                  [ for (idx, qw) in list |> List.indexed do
                    ...
                    yield!
                        MainTemplates.mediaArea
                            packageKey
                            isLoading
                            (fun v -> QwMediaUrlChanged(packageKey.Key, v) |> dispatch)
                            (fun v -> QwMediaTypeChanged(packageKey.Key, v) |> dispatch)
                            (QwMediaChanged >> dispatch)
                            (fun _ -> (QwMediaClear packageKey.Key) |> dispatch)
                            settings.MediaHost
                            slip.QuestionMedia
                            "Clear" ]
          ...
          td
              ...
              [ div
                    ...
                    [ input
                          [ Class "input"
                            Type "number"
                            valueOrDefault (slip.Points.ToString())
                            MaxLength 4.
                            ReadOnly isLoading
                            OnChange(fun ev -> QwPointsChanged(packageKey.Key, ev.Value) |> dispatch) ] ]
                str "Jeopardy"
                div
                    [ Class "control" ]
                    [ input
              ...
              [ div
                    ...
                    [ p
                      ...
                      p
                          ...
                          [ button
                                [ classList
                                      [ "button", true; "has-text-weight-semibold", true; "is-loading", isLoading ]
                                  OnClick(fun _ -> dispatch <| AppendQwToMultiple tourIdx) ]
                                [ str "Append Question To Slip" ] ] ] ]
          td [] []
