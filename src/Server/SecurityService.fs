module rec OpenQuiz.SecurityService
...
let getClaimsFromToken (token: string) =
    if String.IsNullOrEmpty token then
        ...
        try
            ...
            jwt.Claims |> Seq.map (fun claim -> claim.Type, claim.Value) |> Map.ofSeq
        with ex ->
            [ ("ERROR", ex.Message) ] |> Map.ofList
...
let private createRefreshToken () =
    { Value = generateRandomToken ()
      Expired = DateTime.UtcNow.AddDays 7.0 }
    : RefreshToken
            ...
            | Ok principal ->
                let res =
                    async {
                        ...
                        | Ok refreshToken when refreshToken.Expired < DateTime.UtcNow ->
...
let authorizeExpert secret (f: string -> string -> 'arg -> ARES<'res>) =
    authorize secret CustomRoles.Expert
    <| fun principal req ->
...
let authorizeAdmin secret (f: string -> 'arg -> ARES<'res>) =
    authorize secret CustomRoles.Admin
    <| fun principal req ->
        let quizIdStr = principal.FindFirstValue CustomClaims.QuizId
        f quizIdStr req
...
let authorizeTeam secret (f: int -> TeamKey -> 'arg -> ARES<'res>) =
    authorize secret CustomRoles.Team
    <| fun principal req ->
        result {
...
let authorizePrivateReg secret (f: string -> 'arg -> ARES<'res>) =
    authorize secret CustomRoles.Reg
    <| fun principal req ->
        let quizIdStr = principal.FindFirstValue CustomClaims.QuizId
        f quizIdStr req

let authorizeAudience secret (f: string -> 'arg -> ARES<'res>) =
    authorize secret CustomRoles.Aud
    <| fun principal req ->
        let quizIdStr = principal.FindFirstValue CustomClaims.QuizId
        f quizIdStr req
            ...
            | Ok info ->
                ...
                let! _ =
                    ...
                    | Ok exp when exp.Username <> info.Username || exp.Name <> info.Name ->
                        ...
                        Data2.Experts.update env exp.Id logic |> AsyncResult.map ignore
                    | _ -> AsyncResult.retn ()
                ...
                let claims =
                    [ Claim(CustomClaims.UserId, info.Sub)
                      ...
                      if (privateQuizId.IsSome) then
                          Claim(CustomClaims.QuizId, privateQuizId.Value.ToString()) ]
...
let loginAdminUser env secret quizId token =
    ...
    |> AR.bind (fun quiz ->
        if quiz.AdminToken = System.Web.HttpUtility.UrlDecode token then
            let claims =
                [ Claim(CustomClaims.Role, CustomRoles.Admin)
                  Claim(CustomClaims.QuizId, quiz.QuizId.ToString()) ]
            ...
            Error "Wrong entry token" |> AR.fromResult)
        ...
        if team.EntryToken = token then
            ...
            |> AR.bind (fun quiz ->
                ...
                if team.ActiveSessionId = 0 then
                    Data2.Teams.update env { QuizId = quizId; TeamId = teamId } (fun team ->
                        team
                        |> Domain.Teams.dsc (fun dsc -> { dsc with ActiveSessionId = sessionId } |> Ok))
                    |> AR.next (AR.retn ())
                else
                    AR.retn ()
                |> AR.next (loginResp env secret claims (TeamUser user)))
        else
            Error "Authenticaton Error" |> AR.fromResult)
...
let loginRegUser env secret quizId token =
    ...
    |> AR.bind (fun quiz ->
        if quiz.RegToken = System.Web.HttpUtility.UrlDecode token then
            let claims =
                [ Claim(CustomClaims.Role, CustomRoles.Reg)
                  Claim(CustomClaims.QuizId, quiz.QuizId.ToString()) ]
...
let loginAudUser env secret quizId token =
    ...
    |> AR.bind (fun quiz ->
        if quiz.ListenToken = System.Web.HttpUtility.UrlDecode token then
            let claims =
                [ Claim(CustomClaims.Role, CustomRoles.Aud)
                  Claim(CustomClaims.QuizId, quiz.QuizId.ToString()) ]
