...
module Experts =
    ...
    let addQuiz quizId (expert: Expert) =
        { expert with
            Quizzes = quizId :: expert.Quizzes }

    let addPackage packageId (expert: Expert) =
        { expert with
            Packages = packageId :: expert.Packages }
        |> Ok

    let removePackage packageId (expert: Expert) =
        { expert with
            Packages = expert.Packages |> List.filter (fun id -> id <> packageId) }
        |> Ok

    let removeQuiz quizId (expert: Expert) =
        { expert with
            Quizzes = expert.Quizzes |> List.filter (fun id -> id <> quizId) }
        |> Ok
    ...
    let addComp quizId teamId (expert: Expert) =
        { expert with
            Competitions = expert.Competitions.Add(quizId, teamId) }

    let addSharedPackage packageId (expert: Expert) =
        { expert with
            PackagesSharedWithMe = (packageId :: expert.PackagesSharedWithMe) |> List.distinct }
        |> Ok

    let removeSharedPackage packageId (expert: Expert) =
        { expert with
            PackagesSharedWithMe = expert.PackagesSharedWithMe |> List.filter (fun id -> id <> packageId) }
        |> Ok
...
type Slip =
    ...
    member x.QuestionsPartCount qwIdx =
        ...
        | Multiple(_, slips) ->
            slips
            |> List.tryItem qwIdx
            |> Option.map (fun slip -> slip.QuestionsCount)
            |> Option.defaultValue 0

    member x.FirstSingleSlip =
        match x with
        | Single slip -> Some slip
        | Multiple(_, slips) -> slips |> List.tryHead
...
type SingleSlip =
    ...
    member x.QuestionSeconds =
        x.Seconds
        |> Option.bind (fun seconds ->
            match seconds >= x.QuestionsCount with
            | true -> seconds / x.QuestionsCount |> Some
            | false -> None)
...
module Packages =
    ...
    let shareWith (expertId: string) (package: Package) =
        { package with
            SharedWith = (expertId :: package.SharedWith) |> List.distinct }
        |> Ok

    let removeShareWith (expertId: string) (package: Package) =
        { package with
            SharedWith = package.SharedWith |> List.filter ((<>) expertId) }
        |> Ok
...
type QuizTour =
    ...
    member x.NextQw() =
        { x with
            QwIdx =
                if x.QwIdx + 1 < x.Slip.QuestionsCount then
                    x.QwIdx + 1
                else
                    x.Slip.QuestionsCount
            QwPartIdx = 0 }
    ...
    member x.NextQwPart() =
        ...
        { x with
            QwPartIdx = min nextIdx maxIdx }

    member x.SetNextQwIndex idx =
        { x with
            QwIdx =
                if idx < x.Slip.QuestionsCount then
                    idx
                else
                    x.Slip.QuestionsCount }
    ...
    member x.SetNextQwPartIndex qwIdx idx =
        { x with
            QwIdx =
                if idx < (x.Slip.QuestionsPartCount qwIdx) then
                    idx
                else
                    (x.Slip.QuestionsPartCount qwIdx) }
...
type Quiz =
    ...
    member this.CurrentTour = List.tryHead this.Tours
    member this.CurrentTourIndex = List.length this.Tours - 1
    member this.GetTour index = this.Tours |> List.tryItem index
...
module Quizzes =
    ...
    let authorize expertId (quiz: QuizDescriptor) =
        if quiz.Producer <> expertId then
            Error "Quiz is produced by someone else"
        else
            Ok quiz
    ...
    let getNextTourSeconds (slip: Slip) (quiz: Quiz) =
        slip.FirstSingleSlip
        |> Option.bind (fun slip -> slip.QuestionSeconds)
        |> Option.defaultWith (fun () ->
            quiz.CurrentTour
            |> Option.bind (fun tour -> tour.Slip.FirstSingleSlip |> Option.bind (fun slip -> slip.QuestionSeconds))
        ...
        { quiz with
            Tours =
                { Name = getNextTourName slip quiz
                  Seconds = getNextTourSeconds slip quiz
                  Status = Announcing
                  Slip = slip
                  QwIdx = 0
                  QwPartIdx = 0
    ...
    let changeStatus status (pkgProvider: Provider<int, Package>) (quiz: Quiz) =
        match
            { quiz with
                Dsc = { quiz.Dsc with Status = status } }
        with
    ...
    let setQuestionPartIdx qwIdx qwPartIdx (quiz: Quiz) =
        quiz
        |> updateCurrentTour (fun tour -> tour.SetNextQwPartIndex qwIdx qwPartIdx)
        |> Ok

    let update qwName seconds pkgQwIdx qwIdx qwPartIdx slip (quiz: Quiz) =
        { quiz with
            Dsc = { quiz.Dsc with PkgSlipIdx = pkgQwIdx } }
    ...
    let showMedia (quiz: Quiz) =
        quiz
        |> updateCurrentTour (fun tour -> { tour with IsMediaDisplayed = true })
        |> Ok

    let showQuestion (quiz: Quiz) =
        quiz
        |> updateCurrentTour (fun tour -> { tour with IsQuestionDisplayed = true })
        |> Ok
...
type TeamKey = { QuizId: int; TeamId: int }
...
type Team =
    ...
    member x.GetAnswer qwKey = x.Answers |> Map.tryFind qwKey
...
module Teams =
    ...
    let validateTeamUpdate isNewTeam (teamName: string) (teamsInQuiz: TeamDescriptor list) (quiz: QuizDescriptor) =
        ...
        | _ when
            teamsInQuiz
            |> List.exists (fun t -> t.Name.Equals(teamName, StringComparison.InvariantCultureIgnoreCase))
            ->
            Some "Team with such name is alreay registered"
        | _ -> None

    let changeName newName (team: Team) =
        { team with
            Dsc = { team.Dsc with Name = newName } }
    ...
    let registerAnswer qwIndex awText jeopardy now (team: Team) =
        if String.IsNullOrWhiteSpace awText then
            Error "Answer is empty"
        else
            let awText =
                if awText.Length <= 256 then
                    awText
                else
                    awText.Substring(0, 256)

            match team.Answers.TryFind qwIndex with
            | Some aw when aw.Text <> "" -> Error <| "Answer is alredy registered: " + aw.Text
            | None
            | Some _ -> // if answer is exists but Text is empty, allow reset the answer
                Ok
                    { team with
                        Answers =
                            team.Answers.Add(
    ...
    let setResultWithoutAnswer qwIdx res now (team: Team) =
        ...
        | None ->
            ...
            { team with
                Answers = team.Answers.Add(qwIdx, aw) },
            true

type TeamDetail = { Result: decimal option; Vote: int }

type TourResult = { Points: decimal }
...
module Results =
    ...
    let private correctAnswersAndVotes qwKey (teams: Team list) =
        teams
        |> List.fold
            (fun (score, votes) team ->
                match team.Answers.TryGetValue qwKey with
                | true, aw ->
                    score
                    + (aw.Result
                       |> Option.map (fun r -> if r > 0m then 1 else 0)
                       |> Option.defaultValue 0),
                    votes + aw.VotePoints
                | _ -> (score, votes))
            (0, 0)
    ...
    let questionResults (quiz: Quiz) teamsCount teams =
        ...
        |> List.mapi (fun tourIdx tour ->
            ...
            | Single slip ->
                ...
                [ { Key = key
                    Name = questionName tour 0
                    EOT = slip.EndOfTour
                    Score = teamsCount - score
                    Votes = votes
                    Rating = 0m } ]
            | Multiple(_, slips) ->
    ...
    let teamToursResults (quiz: Quiz) (team: Team) =
        ...
        [ for (idx, tour) in quiz.Tours |> List.rev |> List.mapi (fun idx t -> idx, t) do
              ...
              | Single slip ->
                  ...
                  | Some aw -> tourPoints <- tourPoints + (aw.Result |> Option.defaultValue 0.0m)
                  | None -> ()
    ...
    let teamResults quiz (teams: Team list) (questionResults: QuestionResult list) =
        ...
        [ for ((points, rating), teams) in
              teams
              |> List.groupBy (fun t -> t.Points, (teamRating questionResults t))
        ...
        if teamsCount > 0 then
            ...
            |> List.fold
                (fun state tr ->
                    ...
                    | true, details ->
                        let s =
                            decimal details.Vote
                            * (teamsCount - decimal (tr.PlaceFrom + tr.PlaceTo) / 2m + 1m)
    ...
    let results (quiz: Quiz) (teams: Team list) : Results =
        ...
        { Questions =
            ...
            |> List.map (fun qr ->
                { qr with
                    Rating = questionRating teamsCount teamResults qr.Key })
          Teams = teamResults }