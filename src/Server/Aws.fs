...
let getUserToken env code =
    ...
    async {
        ...
        | _ ->
            (env :> ILog)
                .Logger.Error("{@Op} {@Status} {@Resp}", "Aws.getUserToken", resp.StatusCode, respStr)
...
let private publishQuery =
    sprintf
        """{ "query": "mutation quizMessage {quizMessage(quizId: %i, token: \"%s\", body: \"%s\", version: %i){quizId, token, body, version} }"}"""
...
let getSignedUrl bucketName (cat: MediaCategory) =
    ...
    let key =
        sprintf "%s/%.0f-%s" cat.Prefix (toEpoch DateTime.UtcNow) (generateRandomToken ())
...
let deleteFile env (key: string) : Async<Result<unit, string>> =
    ...
    let req =
        DeleteObjectRequest(BucketName = (env :> ICfg).Configurer.BucketName, Key = key)
