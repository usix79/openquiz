...
let api env (context: HttpContext) : IMainApi =
    ...
    let ex proc f =
        let ff f =
            (fun expertId usersysname username (quizIdStr: string) req ->
                f expertId usersysname username (tryParseInt32 quizIdStr) req)

        SecurityService.exec (env :> ILog).Logger proc
        <| SecurityService.authorizeExpertCheckPrivateQuiz secret (ff f)

    let exPublisher proc f =

        let ff f =
            fun expertId username req ->
                async {
                    match! Data2.Experts.get env expertId with
                    | Ok expert -> return! f expert req
                    | Error _ ->
                        env.Logger.Error("{Op} {Error} {ExpertId}", "main", "Wrong Publisher Id", expertId)
                        return Error "Wrong Publisher Id"
                }

        SecurityService.exec env.Logger proc
        <| SecurityService.authorizeExpert secret (ff f)
...
let becomeProducer env expertId usersysname name _ _ =
    ...
    let logic expert =
        expert |> Domain.Experts.becomeProducer |> Ok

    Data2.Experts.updateOrCreate env expertId logic creator |> AR.map ignore

let createQuiz env expert _ =
    let creator quizId =
        Domain.Quizzes.createNew quizId expert.Id expert.DefaultImg expert.DefaultMixlr

    let logic (quiz: Domain.Quiz) expert =
        expert |> Domain.Experts.addQuiz quiz.Dsc.QuizId |> Ok

    Data2.Quizzes.create env creator
    |> AR.side (fun quiz -> Data2.Experts.update env expert.Id (logic quiz))
    |> AR.map (fun quiz ->
        {| Record = quiz.Dsc |> Main.quizProdRecord
           Card = Main.quizProdCard quiz |})
...
let getProdQuizzes env expert _ =
    ...
    |> Async.map (
        Array.choose (function
            | Ok r -> Some(Main.quizProdRecord r)
            | _ -> None)
    )
        ...
        |> Result.map (fun _ ->
            let dsc =
                { quiz.Dsc with
                    ...
                    FarewellText = card.FarewellText
                    RegText = card.RegText
                    InfoText = card.InfoText
                    WithPremoderation = card.WithPremoderation
                    EventPage = card.EventPage
                    MixlrCode =
                        if card.MixlrCode.Trim() = "" then
                            None
                        else
                            Some card.MixlrCode }
...
let getRegModel env expId username name quizId _ =
    quizId
    |> Result.fromOption "Quiz not defined"
    |> AR.fromResult
    |> AR.bind (fun quizId ->
...
let registerTeam env expId username name quizId req =
    let expCreator () =
        Domain.Experts.createNew expId username name
...
let private createTeam env exp quiz teamName =
    ...
    |> AR.bind (fun teamsInQuiz ->
        ...
        |> AR.side (fun team ->
            let logic expert =
                expert |> Domain.Experts.addComp team.Dsc.QuizId team.Dsc.TeamId |> Ok
...
let private updateTeam env quiz team teamName =
    ...
    | false ->
        let logic teamsInQuiz (team: Domain.Team) =
            result {
                do!
                    match Domain.Teams.validateTeamUpdate false teamName teamsInQuiz quiz with
                    | Some txt -> Error txt
                    | _ -> Ok()
        ...
        Data2.Teams.getDescriptors env quiz.QuizId
        |> AR.bind (fun teamsInQuiz -> Data2.Teams.update env team.Key (logic teamsInQuiz))
...
let createPackage env expert _ =
    ...
    |> AR.map (fun pkg ->
        {| Record = pkg.Dsc |> packageRecord
           Card = Main.packageCard expert.Id (Data2.Experts.provider env) pkg |})
...
let aquirePackage env expert req =
    ...
    |> AR.bind (fun origPkg ->
        ...
        |> AR.side (fun _ ->
            Data2.Experts.update env origPkg.Producer (Domain.Experts.removePackage origPkg.PackageId)))
    |> AR.side (fun pkg -> Data2.Experts.update env expert.Id (Domain.Experts.addPackage pkg.Dsc.PackageId))
...
let updateSettings env expert req =
    let logic (exp: Domain.Expert) =
        { exp with
            DefaultImg = req.DefaultImg
            DefaultMixlr = req.DefaultMixlr }
        |> Ok

    Data2.Experts.update env expert.Id logic |> AR.map Main.settingsCard
